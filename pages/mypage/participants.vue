        ...
        <div class="container">
            ...
            <div class="wrap">
                <div class="m-tabs type03">
                    <nuxt-link to="/mypage/participants" class="m-tab active">내가 참여한 행사</nuxt-link>
                    <nuxt-link to="/mypage/meetings" class="m-tab">내가 만든 행사</nuxt-link>
                </div>
    ...
    methods: {
        loadMore(state) {
            if(this.items.meta.current_page <= this.items.meta.last_page){
                ...
                this.$axios.get("/api/participants", {
                    params: this.form
                }).then(response => {
                    this.items = {
                        ...response.data,
